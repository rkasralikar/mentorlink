...
REGION=us-west-2
REPOSITORY_NAME=bigdatauser
CLUSTER=staging
FAMILY=`sed -n 's/.*"family": "\(.*\)",/\1/p' usertaskdef.json | tr -d '\r'`
NAME=`sed -n 's/.*"name": "\(.*\)",/\1/p' usertaskdef.json | tr -d '\r'`
...
sed -e "s;%BUILD_ID%;${BUILD_ID};g" -e "s;%REPOSITORY_URI%;${REPOSITORY_URI};g" usertaskdef.json > ${NAME}-v_${BUILD_ID}.json
