...
REGION=us-west-2
REPOSITORY_NAME=chatservice
CLUSTER=staging
FAMILY=`sed -n 's/.*"family": "\(.*\)",/\1/p' chattaskdef.json | tr -d '\r'`
NAME=`sed -n 's/.*"name": "\(.*\)",/\1/p' chattaskdef.json | tr -d '\r'`
SERVICE_NAME=dev-test-chatservice1
BUILD_ID=$1
env
...
sed -e "s;%BUILD_ID%;${BUILD_ID};g" -e "s;%REPOSITORY_URI%;${REPOSITORY_URI};g" chattaskdef.json > ${NAME}-v_${BUILD_ID}.json
